...
import { treatmentMappingSchema } from '@/validators/tooth-chart.validator';
...
export const POST = withApiHandler(
  withAuth(async (request: NextRequest, context, auth) => {
    ...
    const body = await parseJsonBody(request, treatmentMappingSchema);
    const chart = await toothChartService.addTreatmentMappings(id, body, auth);
    return successResponse(chart, 'Treatment mappings added successfully');
